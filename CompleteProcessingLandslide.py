...
class ProjectLandslide(object):
    ...
    def __init__(self, dbname, user, password):
        ...
        self.landslide_frequency_tifs_dir = "C:/sparc/input_data/landslides/ls_pr_20160129163540_tiff/"
        os.chdir(self.landslide_frequency_tifs_dir)
        self.landslide_monthly_tifs = glob.glob("*.tif")

    def admin_2nd_level_list(self, paese):

        ProjectLandslide.paese = paese
    ...
    def administrative_level_0_1_fetch(self, code_admin):
        ...
        for row in self.cur:
            ProjectLandslide.ISO2_code = row[0]
            ProjectLandslide.ISO3_code = row[1]
            ProjectLandslide.ADM0_GAUL_code = row[2]
            ProjectLandslide.ADM0_GAUL_name = row[3]
            ProjectLandslide.ADM1_GAUL_name = row[4]
            ProjectLandslide.ADM1_GAUL_code = row[5]

        dict_all_codes = {}
        dict_all_codes[code_admin] = {}
        dict_all_codes[code_admin]['iso2'] = ProjectLandslide.ISO2_code
        dict_all_codes[code_admin]['iso3'] = ProjectLandslide.ISO3_code
        dict_all_codes[code_admin]['adm0_code'] = ProjectLandslide.ADM0_GAUL_code
        dict_all_codes[code_admin]['adm0_name'] = ProjectLandslide.ADM0_GAUL_name
        dict_all_codes[code_admin]['adm1_code'] = ProjectLandslide.ADM1_GAUL_code
        dict_all_codes[code_admin]['adm1_name'] = ProjectLandslide.ADM1_GAUL_name
...
class HazardAssessmentLandslide(ProjectLandslide):
    ...
    def statistics_landslide_zones(self):
        ...
        for dr_temp in self.adm2_landslide_months:
            ...
            if sum_val_fld > 0 and sum_val_pop > 0:
                ...
                template_dbf = "C:/sparc/input_data/landslide/template_pop_stat.dbf"
                ...
                try:
                    ...
                    print e.message
        return "People in landslide areas....\n"

    def cut_rasters_landslide(self, paese, name_admin, code):

        global admin_vect
        admin_vect = self.proj_dir + paese + "/" + name_admin + "_" + str(code) + "/" + name_admin + ".shp"

        ProjectLandslide.cur = self.conn.cursor()
        ...
        ProjectLandslide.cur.execute(comando)
        for row in ProjectLandslide.cur:
        ...
        #CUT and SAVE Population and landslide Month/Season for each admin2 area
        self.adm2_landslide_months = []
        if self.population_raster!= "None":
            ...
            try:
                ...
                contatore = 1
                for raster in self.landslide_monthly_tifs:
                    rst_file = self.landslide_frequency_tifs_dir + raster
                    try:
                        landslide_out = self.proj_dir + paese + "/" + name_admin + "_" + str(code) + "/" + name_admin + "_land_freq" + str(contatore) + ".tif"
                        self.adm2_landslide_months.append(landslide_out)
                        arcpy.gp.ExtractByMask_sa(arcpy.Raster(rst_file), admin_vect, landslide_out)
                        contatore += 1
                    except:
                        return "No landslide Raster"
            except:
                return "No Population Raster"
            self.statistics_landslide_zones()
        else:
            return "Problem cutting Population Raster"

class ManagePostgresDBLandslide(ProjectLandslide):
    ...
    def delete_monthly_landslide_table_if_exists(self):

        comando_delete_table = "DROP TABLE " + self.schema + ".sparc_population_month_landslide CASCADE;"
        try:
    ...
    def create_sparc_landslide_population_month(self):
        ...
        campi = """(
           ...
           freq integer,
           pop integer,"""
        constraint = "CONSTRAINT pop_month_landslide_pkey PRIMARY KEY (id));"

        try:
            comando = SQL % (self.schema, 'sparc_population_month_landslide', campi, constraint)
            self.cur.execute(comando)
            print "Monthly Population landslide Table Created"
    ...
    def collect_landslide_population_frequencies_frm_dbfs(self):

        direttorio = self.dirOutPaese

        dct_valori_landslide = {}
            ...
            if direttorio_principale != direttorio:
                ...
                for file in files_dbf:
                    ...
                    try:
                        ...
                        tabella.open()
                        dct_valori_landslide[unique_id] = {}
                        for recordio in tabella:
                            if recordio.value > 0:
                                dct_valori_landslide[unique_id][recordio['value']] = recordio['sum']
                    except:
                        pass
        ...
        for single_adm_chiavi, single_adm_value in sorted(dct_all_admin_values.iteritems()):
            for adm2_landslide_keys, adm2_landslide_values in sorted(dct_values_annual_landslide.iteritems()):
                ...
                if single_adm_chiavi == val_adm:
                    ...
                    for linee_con_corrispondenza_amministrativa in dct_values_annual_landslide.iteritems():
                        if linee_con_corrispondenza_amministrativa[0] == adm2_landslide_keys:
                            if len(linee_con_corrispondenza_amministrativa[1].keys()) > 0:
                                for dove_ci_sono_persone in linee_con_corrispondenza_amministrativa[1].iteritems():
                                    linea_calc = "' '," + str(dove_ci_sono_persone[0]) + "," + str(dove_ci_sono_persone[1])
                                    linee.append(linea_adm + "," + linea_calc)

        inserimento_mensili = []
        for linea in linee:
            #print linea,len(linea)
            inserimento = "INSERT INTO public.sparc_population_month_landslide" + \
    ...
    def clean_old_values_month_landslide(self, paese):

        conteggio = "SELECT COUNT (iso3) FROM sparc_population_month_landslide WHERE adm0_name = '" + paese + "';"

        if conteggio>0:
            sql_clean = "DELETE FROM sparc_population_month_landslide WHERE adm0_name = '"+ paese + "';"
            self.cur.execute(sql_clean)

    def insert_landslide_in_postgresql(self, lista_inserimento):
